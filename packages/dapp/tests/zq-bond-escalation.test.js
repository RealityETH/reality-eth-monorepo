...
test.describe('q: bond escalation', () => {
  test.setTimeout(45000);
  let snap;
  let fixtures;

  test.beforeAll(async () => { fixtures = await createBondEscalationFixtures(); });
  ...
  test('submitting with too-low bond shows is-error and does not send a transaction', async ({ page }) => {
    ...
    await win.locator('select[name="input-answer"]').selectOption('0');
    const bondField = win.locator('input[name="questionBond"]');
    await bondField.fill('0.001');
