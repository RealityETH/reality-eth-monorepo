...
$('div.loadmore-button').on('click', async function(e) {
    ...
    const [old_max_store, next_question_ids] = handleLoadMore(sec);
    for(const nqid in next_question_ids) {
        // Mostly these will come direct from the cache
        const [next_ctr, next_question_id] = parseContractQuestionID(nextid);
        let question = await ensureQuestionDetailFetched(next_ctr, nqid.nextid, (60*60*1000));
        populateSection(sec, question, nqid.previd, ARBITRATOR_FAILED_BY_CONTRACT, ARBITRATOR_LIST_BY_CONTRACT);
    }
...
function filledQuestion(item, fetched_ms) {
    ...
    // RPC TODO: Add this to RPC
    question.decimals = TOKEN_INFO[TOKEN_TICKER]['decimals'];
    question.small_number = ""+TOKEN_INFO[TOKEN_TICKER]['small_number'];
...
//    question = await ensureQuestionDetailFetched(contract, question_id, 1, 1, item.blockNumber, -1)
    ...
    if (is_finalized) {
        ...
        if (insert_before !== -1) {
            ...
            populateSection('questions-resolved', question, insert_before, ARBITRATOR_FAILED_BY_CONTRACT, ARBITRATOR_LIST_BY_CONTRACT);
            $('#questions-resolved').find('.scanning-questions-category').css('display', 'none');
            if (countDisplayEntries('questions-resolved') > 3 && $('#questions-resolved').find('.loadmore-button').css('display') == 'none') {
        ...
        if (insert_before !== -1) {
            populateSection('questions-upcoming', question, insert_before, ARBITRATOR_FAILED_BY_CONTRACT, ARBITRATOR_LIST_BY_CONTRACT);
            $('#questions-upcoming').find('.scanning-questions-category').css('display', 'none');
            if (countDisplayEntries('questions-upcoming') > 3 && $('#questions-upcoming').find('.loadmore-button').css('display') == 'none') {
        ...
        if (!question.is_pending_arbitration) {
            const insert_before = update_ranking_data('questions-active', contractQuestionID(question), calculateActiveRank(created, question.bounty, question.bond, ""+TOKEN_INFO[TOKEN_TICKER]['small_number']), 'desc');
            if (insert_before !== -1) {
                populateSection('questions-active', question, insert_before, ARBITRATOR_FAILED_BY_CONTRACT, ARBITRATOR_LIST_BY_CONTRACT);
                $('#questions-active').find('.scanning-questions-category').css('display', 'none');
                if (countDisplayEntries('questions-active') > 3 && $('#questions-active').find('.loadmore-button').css('display') == 'none') {
        ...
        if (isAnswered(question)) {
            ...
            if (insert_before !== -1) {
                populateSection('questions-closing-soon', question, insert_before, ARBITRATOR_FAILED_BY_CONTRACT, ARBITRATOR_LIST_BY_CONTRACT);
                $('#questions-closing-soon').find('.scanning-questions-category').css('display', 'none');
                if (countDisplayEntries('questions-closing-soon') > 3 && $('#questions-closing-soon').find('.loadmore-button').css('display') == 'none') {
    ...
    reflectDisplayEntryChanges(IS_INITIAL_LOAD_DONE);
...
function displayQuestionDetail(question_detail) {
    ...
    if (rcqa.length) {
        ...
        rcqa.find('.rcbrowser__close-button').on('click', function() {
            let parent_div = $(this).closest('div.rcbrowser.rcbrowser--qa-detail');
            closeRCQuestionWindow(parent_div, contract_question_id, rcqa);
        });
...
// 
    fetchAndDisplayQuestionFromGraph(RC_DISPLAYED_CONTRACTS, 'questions-active-answered', offset, countDisplayMaxStore('questions-active')); 
    fetchAndDisplayQuestionFromGraph(RC_DISPLAYED_CONTRACTS, 'questions-active-unanswered', offset, countDisplayMaxStore('questions-active')); 
    fetchAndDisplayQuestionFromGraph(RC_DISPLAYED_CONTRACTS, 'questions-upcoming', offset, countDisplayMaxStore('questions-upcoming')); 
    fetchAndDisplayQuestionFromGraph(RC_DISPLAYED_CONTRACTS, 'questions-resolved', offset, countDisplayMaxStore('questions-resolved')); 
    fetchAndDisplayQuestionFromGraph(RC_DISPLAYED_CONTRACTS, 'questions-awaiting-arbitration', offset, countDisplayMaxStore('questions-upcoming')); 
};
...
async function fetchAndDisplayQuestionFromGraph(displayed_contracts, ranking, offset, max_store) {
    console.log('fetchAndDisplayQuestionFromGraph', displayed_contracts, ranking, offset, max_store);
