...
See [creating-templates](#creating-templates) for how and why you can create your own custom template.
...
If the arbitrator you have selected charges a per-question fee, you must supply at least this much ETH. Any additonal ETH you send with this call will be assigned as a reward to whoever supplies the final answer to the question. See [fees](fees.md) for more information.

See [templates](#templates) for the `template_id`, and how the `question` parameter is structured.
...
The `arbitrator` is the address of the contract that will be able to arbitrate. See [arbitrators](arbitrators.md) for more information. From version 3, this parameter may be left empty. For prior versions, if you wish to make arbitration impossible, you can instead supply the address of the `reality.eth` contract itself.
