...
const NEYNAR_CONFIG = require('./secrets/config.json');
    ...
    */
    ...
    for (const q of questions) {
        ...
        if (q.currentAnswerTimestamp && parseInt(q.currentAnswerTimestamp) > 0) {
            ...
            const bond = ethers.utils.formatUnits(q.currentAnswerBond, decimals).replace(/\.0+$/,'') + ' ' + token;
            bond_txt = '('+bond+')';
        } else if (q.bounty > 0) {
        ...
        await tweetQuestion(msg_id, title, bond_txt, bounty_txt, current_answer_txt, q.qCategory, q.user, url);
        updateStateFile(chain_id, seen_ts, i);
    }
}

async function tweetQuestion(msg_id, title, bond_txt, bounty_txt, answer_txt, category, creator, url) {
    ...
    const client = new NeynarAPIClient(NEYNAR_CONFIG.NEYNAR_API_KEY);
    try {
