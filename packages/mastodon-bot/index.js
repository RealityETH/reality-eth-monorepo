...
const MASTO_CONFIG = require('./secrets/config.json');
...
const NOOP = noop_arg || ('noop' in MASTO_CONFIG && MASTO_CONFIG['noop']);
...
async function doQuery(graph_url, chain_id, contract_tokens, tokens, initial_ts) {
    ...
    */
    ...
    for (const q of questions) {
        ...
        await tweetQuestion(title, bond_txt, bounty_txt, current_answer_txt, q.qCategory, q.user, url);
        updateStateFile(chain_id, seen_ts, i);
    }
}

async function tweetQuestion(title, bond_txt, bounty_txt, answer_txt, category, creator, url) {

    const masto = await login({
        url: MASTO_CONFIG.url,
        accessToken: MASTO_CONFIG.access_token
    });
