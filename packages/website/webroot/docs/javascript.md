...
This library will help you handle the encoding and decoding of questions using templates. See [contracts](contracts.md) to understand how the template system works.
...
Questions are created by calling the `askQuestion` method of the reality.eth contract. This requires a template ID and some parameters, stored in a string of text. The questions are formatted using an unusual delimiter character, "␟". See [contracts](contracts.md) for more details on formatting. The `encodeText` function will handle formatting text for inclusion into a template.
