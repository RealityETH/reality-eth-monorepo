...
import { ConstantsUtil, NumberUtil, ParseUtil } from '@reown/appkit-common';
import { ChainController, ConnectionController, CoreHelperUtil, EventsController, ModalController, ProviderController, RouterController, SnackController } from '@reown/appkit-controllers';
...
describe('PayController', () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(ApiUtil, 'getQuote').mockResolvedValue({});
        vi.spyOn(ApiUtil, 'getQuoteStatus').mockResolvedValue({ status: 'success' });
    ...
    describe('onTransfer', () => {
        ...
        it('should handle EVM native token transfer successfully', async () => {
            ...
            expect(PaymentUtil.processEvmNativePayment).toHaveBeenCalledWith(nativeParams.paymentAsset, ConstantsUtil.CHAIN.EVM, {
        ...
        it('should handle EVM ERC20 token transfer successfully', async () => {
            ...
            expect(PaymentUtil.processEvmErc20Payment).toHaveBeenCalledWith(mockTransferParams.paymentAsset, {
                recipient: mockTransferParams.toAddress,
                amount: mockTransferParams.amount,
                fromAddress: mockTransferParams.fromAddress
            });
        ...
        it('should handle payment processing errors', async () => {
            vi.spyOn(PaymentUtil, 'processEvmErc20Payment').mockRejectedValueOnce(new Error('Payment failed'));
        ...
        it('should throw error for unsupported chain namespace', async () => {
            const unsupportedParams = {
                ...mockTransferParams,
                chainNamespace: 'unsupported'
            };
            await expect(PayController.onTransfer(unsupportedParams)).rejects.toThrow(new AppKitPayError(AppKitPayErrorCodes.INVALID_CHAIN_NAMESPACE));
            expect(PayController.state.currentPayment?.status).toBe('FAILED');
        });
        it('should throw error if target network not found', async () => {
            ...
            await expect(PayController.onTransfer(mockTransferParams)).rejects.toThrow('Target network not found');
            expect(PayController.state.currentPayment?.status).toBe('FAILED');
        });
    ...
    describe('onSendTransaction', () => {
        ...
        it('should handle transaction errors', async () => {
            vi.spyOn(ConnectionController, 'sendTransaction').mockRejectedValueOnce(new Error('Transaction failed'));
            await expect(PayController.onSendTransaction({
                namespace: ConstantsUtil.CHAIN.EVM,
                transactionStep: mockTransactionStep
    ...
    describe('fetchQuote', () => {
        ...
        it('should fetch quote successfully', async () => {
            vi.spyOn(ApiUtil, 'getQuote').mockResolvedValue(mockQuote);
        ...
        it('should set isFetchingQuote to true during fetch', async () => {
            vi.spyOn(ApiUtil, 'getQuote').mockImplementation(() => new Promise(resolve => {
                expect(PayController.state.isFetchingQuote).toBe(true);
                setTimeout(() => resolve(mockQuote), 10);
            }));
            await PayController.fetchQuote(mockQuoteParams);
            expect(PayController.state.isFetchingQuote).toBe(false);
        });
        ...
        it('should generate exchange URL when selectedExchange has transfer step', async () => {
            ...
            PayController.state.selectedExchange = mockExchanges[0];
            vi.spyOn(ApiUtil, 'getQuote').mockResolvedValue(quoteWithSteps);
        ...
        it('should handle API errors', async () => {
            ...
            await expect(PayController.fetchQuote(mockQuoteParams)).rejects.toThrow(new AppKitPayError(AppKitPayErrorCodes.UNABLE_TO_GET_QUOTE));
            expect(PayController.state.quoteError).toBe(AppKitPayErrorMessages.UNABLE_TO_GET_QUOTE);
            expect(SnackController.showError).toHaveBeenCalledWith(AppKitPayErrorMessages.UNABLE_TO_GET_QUOTE);
            expect(PayController.state.isFetchingQuote).toBe(false);
        });
    ...
    describe('fetchQuoteStatus', () => {
        ...
        it('should handle different quote statuses from API', async () => {
            ...
            for (const statusObj of statuses) {
                vi.spyOn(ApiUtil, 'getQuoteStatus').mockResolvedValue(statusObj);
                await PayController.fetchQuoteStatus({ requestId: 'test-request-id' });
        ...
        it('should handle API errors', async () => {
            ...
            await expect(PayController.fetchQuoteStatus({ requestId: 'test-request-id' })).rejects.toThrow(new AppKitPayError(AppKitPayErrorCodes.UNABLE_TO_GET_QUOTE_STATUS));
            expect(PayController.state.quoteStatus).toBe('failure');
        });
    });
