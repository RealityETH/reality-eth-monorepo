...
import { AssetUtil, ChainController, ConstantsUtil, RouterController, SendController, SnackController } from '@reown/appkit-controllers';
...
describe('W3mWalletSendView - parameters handling', () => {
    ...
    it('should handle unsupported chain namespace', async () => {
        ...
        await fixture(html `<w3m-wallet-send-view></w3m-wallet-send-view>`);
        await vi.waitFor(() => {
            expect(SnackController.showError).toHaveBeenCalledWith('Chain "unsupported" is not supported for send parameters');
        });
        expect(BalanceUtil.fetchERC20Balance).not.toHaveBeenCalled();
        expect(SendController.setToken).not.toHaveBeenCalled();
    });
...
//# sourceMappingURL=w3m-wallet-send-view-params.test.js.map