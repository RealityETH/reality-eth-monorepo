...
describe('W3mEmailLoginView', () => {
    beforeEach(() => {
        ...
        vi.spyOn(ConnectorController, 'state', 'get').mockReturnValue({
            ...
            connectors: [MOCK_AUTH_CONNECTOR]
        });
        vi.spyOn(ConnectorController, 'getAuthConnector').mockReturnValue(MOCK_AUTH_CONNECTOR);
        vi.spyOn(ConnectorController, 'subscribeKey').mockImplementation(vi.fn().mockReturnValue(vi.fn()));
    });
    it('should render', async () => {
        const element = await fixture(html `<w3m-email-login-view></w3m-email-login-view>`);
    ...
    it('should throw when email is disabled', async () => {
        ...
        await expect(fixture(html `<w3m-email-login-view></w3m-email-login-view>`)).rejects.toThrowError(/Email is not enabled/);
    });
    it('should throw when no auth connector provided', async () => {
        vi.spyOn(ConnectorController, 'state', 'get').mockReturnValue({
            ...ConnectorController.state,
            connectors: [{ type: 'injected', chain: 'eip155' }]
        });
        await expect(fixture(html `<w3m-email-login-view></w3m-email-login-view>`)).rejects.toThrowError(/No auth connector provided/);
    });
    it('should throw when auth connector is not for supported chain', async () => {
        vi.spyOn(ConnectorController, 'state', 'get').mockReturnValue({
            ...ConnectorController.state,
            connectors: [{ type: AppKitConstantsUtil.CONNECTOR_TYPE_AUTH, chain: 'unsupported' }]
        });
        await expect(fixture(html `<w3m-email-login-view></w3m-email-login-view>`)).rejects.toThrowError(/No auth connector provided/);
    });
...
//# sourceMappingURL=w3m-email-login-view.test.js.map