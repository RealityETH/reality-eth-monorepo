...
describe('W3mRouterContainer', () => {
    ...
    it('should apply transition CSS variables on firstUpdated and updates on prop change', async () => {
        const el = (await fixture(html `<w3m-router-container
    ...
    it('should compute viewDirection and calls setView on history change', async () => {
        ...
        history="A"
      ></w3m-router-container>`);
        expect(el.viewDirection).toBe('');
        el.history = 'A,B';
        await elementUpdated(el);
        expect(el.viewDirection).toContain('next-B');
        await vi.waitFor(() => {
    ...
    it('should render slot content within page-content', async () => {
        const el = (await fixture(html `<w3m-router-container><div id="child">child</div></w3m-router-container>`));
    ...
    it('should set the correct properties and values when mobileFullScreen is true', async () => {
        ...
        const el = (await fixture(html `<w3m-router-container transitionDuration="0s" />`));
        await elementUpdated(el);
        const container = el.shadowRoot?.querySelector('div.container');
        const page = el.shadowRoot?.querySelector('div.page');
    ...
    it('should set the correct properties and values mobileFullScreen is false', async () => {
        ...
        expect(el.style.getPropertyValue('--local-border-bottom-radius').trim()).toBe('var(--apkt-borderRadius-5)');
    });
});
//# sourceMappingURL=w3m-router-container.test.js.map