...
import { ChainController, ConnectionController, ConnectorController, CoreHelperUtil, EventsController, ModalController, OptionsController, RouterController, SnackController, StorageUtil } from '@reown/appkit-controllers';
...
describe('W3mAccountDefaultWidget', () => {
    ...
    describe('Features', () => {
        it('shows fund wallet button when enabled for supported chain', async () => {
            const element = await fixture(html `<w3m-account-default-widget></w3m-account-default-widget>`);
            expect(HelpersUtil.querySelect(element, '[data-testid="w3m-account-default-fund-wallet-button"]')).toBeTruthy();
        });
