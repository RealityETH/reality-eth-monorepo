...
import { ApiController, ChainController, ConnectorController, ConnectorUtil, CoreHelperUtil, StorageUtil } from '@reown/appkit-controllers';
...
describe('W3mConnectorList', () => {
    ...
    it('renders flattened items in correct order', async () => {
        ...
        vi.spyOn(ConnectorController, 'state', 'get').mockReturnValue({
            ...ConnectorController.state,
            connectors: [{ id: 'walletConnect', name: 'WalletConnect', type: 'ANNOUNCED' }]
        });
        const element = await fixture(html `<w3m-connector-list .connectors=${allConnectors}></w3m-connector-list>`);
        element.explorerWallets = [{ id: 'MetaMask', name: 'MetaMask' }];
        await element.updateComplete;
        const items = Array.from(element.shadowRoot?.querySelectorAll('w3m-list-wallet') ?? []);
    ...
    it('should render recent wallets in correct order', async () => {
        vi.spyOn(StorageUtil, 'getRecentWallets').mockReturnValue([
            { id: 'recentX', name: 'Recent From Storage' }
        ]);
        vi.spyOn(ConnectorUtil, 'getConnectorTypeOrder').mockReturnValue(['recent']);
        const element = await fixture(html `<w3m-connector-list .connectors=${[]}></w3m-connector-list>`);
        element.explorerWallets = [];
        await element.updateComplete;
        const items = Array.from(element.shadowRoot?.querySelectorAll('w3m-list-wallet') ?? []);
    ...
    it('renders only specified types in order', async () => {
        ...
        const element = await fixture(html `<w3m-connector-list .connectors=${connectors}></w3m-connector-list>`);
        element.explorerWallets = [{ id: 'MetaMask', name: 'MetaMask' }];
        await element.updateComplete;
        const names = Array.from(element.shadowRoot?.querySelectorAll('w3m-list-wallet') ?? []).map(i => i.getAttribute('name'));
