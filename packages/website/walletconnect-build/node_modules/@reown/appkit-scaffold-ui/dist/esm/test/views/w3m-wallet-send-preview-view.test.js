...
import { AppKitError, ChainController, EventsController, RouterController, SendController } from '@reown/appkit-controllers';
...
describe('W3mWalletSendPreviewView', () => {
    ...
    it('should send SEND_REJECTED event when user rejects request', async () => {
        ...
        button?.click();
        await element.updateComplete;
        viExpect(eventsSpy).toHaveBeenCalledWith(viExpect.objectContaining({ type: 'track', event: 'SEND_REJECTED' }));
    });
    it('should send SEND_ERROR event when random error is thrown', async () => {
        ...
        button?.click();
        await element.updateComplete;
        viExpect(eventsSpy).toHaveBeenCalledWith(viExpect.objectContaining({ type: 'track', event: 'SEND_ERROR' }));
    });
});
