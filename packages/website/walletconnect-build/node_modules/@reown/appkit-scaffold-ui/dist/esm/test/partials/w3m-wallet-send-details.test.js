...
describe('W3mWalletSendDetails', () => {
    ...
    it('should render address and network when provided', async () => {
        const network = { name: 'Ethereum', id: 'eip155:1' };
        const el = (await fixture(html `<w3m-wallet-send-details receiverAddress="0x1234567890abcdef" .caipNetwork=${network} />`));
    ...
    it('should render without network when caipNetwork is undefined', async () => {
        const el = (await fixture(html `<w3m-wallet-send-details receiverAddress="0xabc"></w3m-wallet-send-details>`));
        const networkRow = HelpersUtil.querySelect(el, 'wui-list-content.network');
        expect(networkRow).toBeNull();
    });
});
//# sourceMappingURL=w3m-wallet-send-details.test.js.map