...
import { ChainController, ConnectorController, CoreHelperUtil, OptionsController, RouterController } from '@reown/appkit-controllers';
...
function createMockChainState(chainNamespace, caipNetworkId) {
    ...
    return {
        ...
        activeCaipAddress: mockCaipNetwork
            ? `${mockCaipNetwork.caipNetworkId}:${MOCK_ADDRESS}`
            : undefined
    };
}
const ACCOUNT = {
...
describe('W3mAccountWalletFeaturesWidget', () => {
    ...
    it('it should not return any components if address is not provided in ChainController', async () => {
        ...
        await expect(() => fixture(html `<w3m-account-wallet-features-widget></w3m-account-wallet-features-widget>`)).rejects.toThrow('w3m-account-features-widget: No account provided');
    });
    it('it should return all components if address is provided in AccountsController', async () => {
        ...
        vi.spyOn(ChainController, 'state', 'get').mockReturnValue(createMockChainState(CommonConstantsUtil.CHAIN.EVM, 'eip155:1'));
    ...
    it('should not show tabs for solana namespace', async () => {
        ...
        vi.spyOn(ChainController, 'state', 'get').mockReturnValue(createMockChainState(CommonConstantsUtil.CHAIN.SOLANA, 'solana:mainnet'));
        const element = await fixture(html `<w3m-account-wallet-features-widget></w3m-account-wallet-features-widget>`);
        await elementUpdated(element);
        const tabs = HelpersUtil.querySelect(element, 'wui-tabs');
...
describe('wallet features visibility', () => {
    ...
    describe('evm wallet features', () => {
        ...
        it('should not show fund wallet if payWithExchange, onramp and receive are disabled', async () => {
            ...
            const element = await fixture(html `<w3m-account-wallet-features-widget></w3m-account-wallet-features-widget>`);
            const fundWalletButton = HelpersUtil.getByTestId(element, 'wallet-features-fund-wallet-button');
    ...
    describe('bitcoin wallet features', () => {
        beforeEach(() => {
            vi.spyOn(ChainController, 'state', 'get').mockReturnValue(createMockChainState(CommonConstantsUtil.CHAIN.BITCOIN, 'bip122:000000000019d6689c085ae165831e93'));
            vi.spyOn(ChainController, 'getAccountData').mockReturnValue({
                ...ChainController.getAccountData(),
