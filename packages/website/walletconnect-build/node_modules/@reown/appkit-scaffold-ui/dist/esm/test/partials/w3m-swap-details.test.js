...
describe('WuiSwapDetails', () => {
    ...
    it('should not render if source token, to token or input error exists', async () => {
        ...
        const element = await fixture(html `<w3m-swap-details></w3m-swap-details>`);
    ...
    it('should display network cost correctly', async () => {
        ...
        await elementUpdated(element);
        const networkCostText = Array.from(element.shadowRoot?.querySelectorAll('wui-text') || []).find(text => text.textContent?.includes('$5'));
    ...
    it('should calculate and display min received amount correctly', async () => {
        ...
        const slippageText = Array.from(element.shadowRoot?.querySelectorAll('wui-text') || []).find(text => text.textContent?.includes(expectedMinAmount));
        expect(slippageText).not.toBeNull();
    });
});
//# sourceMappingURL=w3m-swap-details.test.js.map