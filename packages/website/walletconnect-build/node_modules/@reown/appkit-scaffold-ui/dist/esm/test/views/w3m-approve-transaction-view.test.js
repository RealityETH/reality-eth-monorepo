...
describe('W3mApproveTransactionView - Basic Rendering', () => {
    ...
    it('should render basic structure with frame container', async () => {
        const element = await fixture(html `<w3m-approve-transaction-view></w3m-approve-transaction-view>`);
    ...
    it('should sync theme on first update', async () => {
        ...
        expect(MOCK_AUTH_CONNECTOR.provider.syncTheme).toHaveBeenCalledWith({
            ...
            w3mThemeVariables: getW3mThemeVariables(MOCK_THEME_SNAPSHOT.themeVariables, MOCK_THEME_SNAPSHOT.themeMode)
        });
    });
});
describe('W3mApproveTransactionView - Iframe Positioning Logic', () => {
    ...
    beforeEach(async () => {
        vi.resetAllMocks();
        mockIframe = document.getElementById(IFRAME_ID);
        vi.spyOn(OptionsController, 'state', 'get').mockReturnValue({
            ...OptionsController.state,
            enableEmbedded: false
        });
        ...
        vi.spyOn(ThemeController, 'getSnapshot').mockReturnValue(MOCK_THEME_SNAPSHOT);
        vi.spyOn(ModalController, 'subscribeKey').mockReturnValue(() => { });
        element = await fixture(html `<w3m-approve-transaction-view></w3m-approve-transaction-view>`);
    ...
    it('should position iframe correctly for desktop view', async () => {
        ...
        await element.updateComplete;
        const resizeObserver = element['bodyObserver'];
        if (resizeObserver) {
            const mockEntries = [
                {
    ...
    it('should position iframe correctly for embedded mode', async () => {
        ...
        vi.spyOn(global, 'setTimeout').mockImplementation((handler) => {
...
//# sourceMappingURL=w3m-approve-transaction-view.test.js.map