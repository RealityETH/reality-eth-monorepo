...
describe('W3mEmailVerifyDeviceView', () => {
    ...
    it('should render', async () => {
        const element = await fixture(html `<w3m-email-verify-device-view></w3m-email-verify-device-view>`);
    ...
    it('should throw if no email provided', async () => {
        ...
        await expect(fixture(html `<w3m-email-verify-device-view></w3m-email-verify-device-view>`)).rejects.toThrowError(/No email provided/);
    });
    it('should throw if no auth connector provided', async () => {
        ...
        await expect(fixture(html `<w3m-email-verify-device-view></w3m-email-verify-device-view>`)).rejects.toThrowError(/No auth connector provided/);
    });
    ...
    it('should handle device approval error and go back', async () => {
        MOCK_AUTH_CONNECTOR.provider.connectDevice.mockRejectedValueOnce(new Error('Device approval failed'));
        await fixture(html `<w3m-email-verify-device-view></w3m-email-verify-device-view>`);
    ...
    it('should show correct instruction text', async () => {
        const element = await fixture(html `<w3m-email-verify-device-view></w3m-email-verify-device-view>`);
        const texts = HelpersUtil.querySelectAll(element, TEXT_SELECTOR);
        const instructionText = Array.from(texts || []).find(text => text.textContent?.includes('Approve the login link we sent to'));
        expect(instructionText).toBeTruthy();
    });
    it('should show expiration message', async () => {
        const element = await fixture(html `<w3m-email-verify-device-view></w3m-email-verify-device-view>`);
        const texts = HelpersUtil.querySelectAll(element, TEXT_SELECTOR);
        const expirationText = Array.from(texts || []).find(text => text.textContent?.includes('The code expires in 20 minutes'));
        expect(expirationText).toBeTruthy();
    });
});
//# sourceMappingURL=w3m-email-verify-device-view.test.js.map