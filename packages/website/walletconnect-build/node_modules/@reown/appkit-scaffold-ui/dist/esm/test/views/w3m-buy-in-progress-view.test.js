...
describe('W3mBuyInProgressView', () => {
    ...
    it('should render', async () => {
        const element = await fixture(html `<w3m-buy-in-progress-view></w3m-buy-in-progress-view>`);
    ...
    it('should render with provider label when provider is selected', async () => {
        ...
        const labelText = Array.from(textElements || []).find(el => el.textContent?.includes('Buy in Test Provider'));
    ...
    it('should show error label when error is true', async () => {
        ...
        await element.updateComplete;
        const textElements = element.shadowRoot?.querySelectorAll('wui-text');
        const errorLabel = Array.from(textElements || []).find(el => el.textContent?.includes('Buy failed'));
    ...
    it('should show default label when no provider is selected', async () => {
        ...
        const element = await fixture(html `<w3m-buy-in-progress-view></w3m-buy-in-progress-view>`);
        const textElements = element.shadowRoot?.querySelectorAll('wui-text');
        const defaultLabel = Array.from(textElements || []).find(el => el.textContent?.includes('Continue in external window'));
    ...
    it('should open provider URL when try again is clicked', async () => {
        ...
        await element.updateComplete;
        element['onTryAgain']();
        expect(openHrefSpy).toHaveBeenCalledWith('https://test-provider.com', 'popupWindow', 'width=600,height=800,scrollbars=yes');
    });
...
//# sourceMappingURL=w3m-buy-in-progress-view.test.js.map