...
describe('StorageUtil', () => {
    ...
    describe('setConnections', () => {
        ...
        it('should merge accounts for existing connectors (non-auth)', () => {
            ...
            expect(connector1?.accounts).toEqual(expect.arrayContaining([
                { address: '0x123...', type: 'eoa' },
                { address: '0x999...', type: 'eoa' }
            ]));
        });
        ...
        it('should handle storage errors gracefully', () => {
            ...
            StorageUtil.setConnections([mockConnection1], 'eip155');
            expect(consoleSpy).toHaveBeenCalledWith('Unable to sync connections to storage', expect.any(Error));
        });
    ...
    describe('getConnections', () => {
        ...
        it('should handle storage errors gracefully', () => {
            ...
            const connections = StorageUtil.getConnections();
            expect(connections).toEqual({});
            expect(consoleSpy).toHaveBeenCalledWith('Unable to get connections from storage', expect.any(Error));
        });
    ...
    describe('deleteAddressFromConnection', () => {
        ...
        it('should handle storage errors', () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('Unable to remove address "0x123..." from connector "connector1" in namespace "eip155"');
        });
    ...
    describe('getDisconnectedConnectorIds', () => {
        ...
        it('should return stored disconnected connector IDs', () => {
            ...
            SafeLocalStorage.setItem(SafeLocalStorageKeys.DISCONNECTED_CONNECTOR_IDS, JSON.stringify(mockDisconnectedIds));
            const result = StorageUtil.getDisconnectedConnectorIds();
    ...
    describe('addDisconnectedConnectorId', () => {
        ...
        it('should handle storage errors gracefully', () => {
            ...
            StorageUtil.addDisconnectedConnectorId('connector1', 'eip155');
            expect(consoleSpy).toHaveBeenCalledWith('Unable to set disconnected connector id "connector1" for namespace "eip155"');
        });
    });
    describe('removeDisconnectedConnectorId', () => {
        ...
        it('should do nothing when namespace not found', () => {
            const initialResult = StorageUtil.getDisconnectedConnectorIds();
            StorageUtil.removeDisconnectedConnectorId('connector1', 'nonexistent');
            const finalResult = StorageUtil.getDisconnectedConnectorIds();
        ...
        it('should handle storage errors gracefully', () => {
            ...
            StorageUtil.removeDisconnectedConnectorId('connector1', 'eip155');
            expect(consoleSpy).toHaveBeenCalledWith('Unable to remove disconnected connector id "connector1" for namespace "eip155"');
        });
    });
    describe('isConnectorDisconnected', () => {
        ...
        it('should return false when namespace not found', () => {
            const result = StorageUtil.isConnectorDisconnected('disconnected1', 'nonexistent');
            expect(result).toBe(false);
        });
        it('should handle storage errors gracefully', () => {
            const consoleSpy = vi.spyOn(console, 'info').mockImplementation(() => { });
            ...
            const result = StorageUtil.isConnectorDisconnected('disconnected1', 'eip155');
            expect(result).toBe(false);
            expect(consoleSpy).toHaveBeenCalledWith('Unable to get disconnected connector id "disconnected1" for namespace "eip155"');
        });
