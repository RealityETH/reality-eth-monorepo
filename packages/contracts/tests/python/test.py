...
class TestRealitio(TestCase):
    ...
    def test_frozen_result(self):
        ...
        freezeTs = self.rc0.functions.freezeTs().call()
        self.assertTrue(freezeTs == 0, "Freeze timestamp should be zero before freeze")
        self.rc0.functions.setFreezeTimestamp(self._block_timestamp()).transact(self._txargs())
        freezeTs = self.rc0.functions.freezeTs().call()
        self.assertTrue(freezeTs > 0, "Freeze timestamp should be not zero after freeze")
