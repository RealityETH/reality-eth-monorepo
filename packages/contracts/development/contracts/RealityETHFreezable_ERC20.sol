...
import {IRealityETHFreezable} from "./IRealityETHFreezable.sol";
...
abstract contract RealityETHFreezable_ERC20 is RealityETH_ERC20_v4_0 {
    ...
    uint32 public freezeTs;

    modifier notFrozen() override {
        if (freezeTs > 0) revert ContractIsFrozen();
        _;
    }
    ...
    function isFinalized(bytes32 question_id) public view override returns (bool) {
        ...
        if (freezeTs > 0 && questions[question_id].finalize_ts >= freezeTs) {
            return false;
        }
