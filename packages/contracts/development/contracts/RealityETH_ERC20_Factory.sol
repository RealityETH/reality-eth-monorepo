...
contract RealityETH_ERC20_Factory {
    ...
    event RealityETH_ERC20_deployed(
        address reality_eth,
        address token,
        uint8 decimals,
        string token_ticker
    ...
    function createInstance(address _token) external {
        require(
            deployments[_token] == address(0),
            "There should only be one deployment per version per token"
        );
        ...
        IRealityETH_ERC20(clone).createTemplate(
